...
class MadaniMariaDbExporter:
    ...
    def export(self, session_result: MadaniSessionResult):
        ...
        for result in session_result.results:
            ...
            attribute_values = [result.time_step, result.adjusted_junction_id, result.adjusted_junction_emit, result.adjusted_junction_leak]
            for pipe in result.pipes:
                attribute_values.append(pipe.flow)

            statement = f'INSERT INTO simulation_results({attribute_names})  VALUES ({attribute_values_question})'
